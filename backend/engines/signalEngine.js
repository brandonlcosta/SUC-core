...
export class SignalEngine {
  ...
  processEvent(event) {
    ...
    const isHighlight = ['beam_flip', 'full_send', 'finish'].includes(
      event.event_type
    );
    if (isHighlight) {
    ...
    const arc_ref =
      event.arc_ref || `${event.athlete_id || 'anon'}_${event.event_type}`;
    ...
    appendMetric('signalEngine.processEvent', Date.now() - t0);
    return signal;
  }

  processBatch(events) {
    const t0 = Date.now();
    const out = events.map(e => this.processEvent(e));
    appendMetric('signalEngine.processBatch', Date.now() - t0);
    return out;
  }
}
