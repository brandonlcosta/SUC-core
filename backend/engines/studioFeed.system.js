/**************************************************
 * Studio Feed Emitter
 * Writes compact feed to backend/outputs/logs/studioFeed.json each update.
 **************************************************/
...
const OUT_PATH = path.join('backend', 'outputs', 'logs', 'studioFeed.json');
...
export function studioFeedSystem() {
  ...
  return {
    ...
    async update(world) {
      ...
      const arcState =
        world.query([ARCS])[0]?.components.get(ARCS) ?? {
          topArc: null,
          arcs: []
        };

      const feed = {
        ticker: scored.slice(-8).map(s => ({
          t: s.type,
          a: s.athlete,
          turf: s.turf,
          tier: s.tier
      ...
      appendMetric('studioFeedSystem.update', Date.now() - t0);
    }
  };
}

export default { studioFeedSystem };
