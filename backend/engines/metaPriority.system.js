  ...
  return {
    ...
    async update(world) {
      ...
      for (const { id, components } of targets) {
        ...
        if (streak >= 3) {
          score += 2;
          reasons.push('streak>=3:+2');
        }

        score = Math.max(1, Math.min(10, score));
        world.addComponent(
          id,
          c(HILITE, { score, tier: tier(score), reasons })
        );
      }

      const dt = (performance.now?.() ?? Date.now()) - t0;
      await appendMetric('metaPriority.update', Math.round(dt));
    }
  };
}

export default { metaPrioritySystem };
